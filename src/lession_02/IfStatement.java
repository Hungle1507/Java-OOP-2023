...
public class IfStatement {
    public static void main(String[] args) {
        final int expectedTime = 7; // final => not want change time
        Scanner scanner = new Scanner(System.in);
        System.out.println("Please input time");
        int arrivalTime = scanner.nextInt();
        boolean isHeOnTime = arrivalTime == expectedTime; // when input time from keyCap (arrivalTime) , if value = 7 then isHeOnTime = true , else value > 7 then isHeOnTime = false
